...
def generate_timetable(course_name):
    try:
        ...
        teacher_session_count = {teacher: 14 for teacher in teachers}
...
def create_timetable(course_name, semester, theory_subjects, practical_subjects, teachers, recess_start, recess_end, start_time, end_time, theory_length, practical_length, subject_combine, teacher_combine, practical_combine, teacher_practical_combine, teacher_session_count):
    ...
    try:
        for day in ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday']:
            ...
            # Create time slots list for the day
            time_slots_list = []
            while current_time < end_time_dt:
                try:
                    next_time = increment_time(current_time, timedelta(minutes=theory_length))
            ...
            # Mandatory slot assignment: Ensure each teacher has at least one lecture at a random time
            assigned_teachers = set()
            random.shuffle(teachers)  # Shuffle teacher order for daily variety
            for teacher in teachers:
                slot_assigned = False
                time_slots = [slot for slot in time_slots_list if 'Recess' not in slot]  # Skip recess slots
                random.shuffle(time_slots)  # Shuffle time slots to randomize mandatory lecture time
                
                for time_slot in time_slots:
                    time_start, time_end = map(lambda x: datetime.strptime(x, '%H:%M').time(), time_slot.split(" - "))
                    
                    if is_teacher_available(teacher, day, time_start, time_end):
                        session_type = random.choice(session_types)
                        subject = random.choice(theory_subjects if session_type == "Theory" else practical_subjects)
                        ...
                        book_teacher(teacher, day, time_start, time_end)
                        teacher_session_count[teacher] -= 1
                        assigned_teachers.add(teacher)
                        slot_assigned = True
                        break

            # Proceed with the rest of the day’s schedule
            for time_slot in time_slots_list:
                if 'Recess' in time_slot:
                    timetable[f"{course_name}-{semester}"][day].append("Recess")
                    continue

                time_start, time_end = map(lambda x: datetime.strptime(x, '%H:%M').time(), time_slot.split(" - "))
                session_type = random.choice(session_types)
                available_teachers = [t for t in teachers if teacher_session_count[t] > 0 and is_teacher_available(t, day, time_start, time_end)]
        ...
        logging.info(f"Error occurred while creating timetable for {course_name}-{semester}")
        raise CustomException(e, sys)
